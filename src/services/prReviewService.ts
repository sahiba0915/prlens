  ...
  try {
    ...
    if (err instanceof GitHubApiError) {
      const missingTokenHint =
        (err.status === 401 || err.status === 403) && !process.env.GITREVIEWPILOT_GITHUB_TOKEN && !process.env.GITHUB_TOKEN;
      if (missingTokenHint) {
        throw new PrReviewError(
          "GITHUB_AUTH_MISSING",
          "Missing GitHub token. Set GITREVIEWPILOT_GITHUB_TOKEN (preferred) or GITHUB_TOKEN.",
          err.details,
          err
        );
      }
