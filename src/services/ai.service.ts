...
export async function analyze(prompt: string, options: AnalyzeOptions = {}): Promise<string> {
  const provider: LlmProvider =
    options.provider ??
    (getEnvFirst("GITREVIEWPILOT_LLM_PROVIDER") as LlmProvider | undefined) ??
    "openai-compatible";
...
class OpenAICompatibleClient implements LlmClient {
  private defaultBaseUrl(): string {
    return normalizeBaseUrl(
      getEnvFirst("GITREVIEWPILOT_LLM_BASE_URL", "OPENAI_BASE_URL") ?? "https://api.openai.com"
    );
  }
  ...
  async analyze(prompt: string, options: AnalyzeOptions = {}): Promise<string> {
    ...
    if (!apiKey) {
      throw missingKeyError(
        provider,
        "Set GITREVIEWPILOT_LLM_API_KEY (preferred) or OPENAI_API_KEY, or use a localhost base URL."
      );
    }
...
class GeminiClient implements LlmClient {
  private defaultBaseUrl(): string {
    return normalizeBaseUrl(
      getEnvFirst("GITREVIEWPILOT_LLM_BASE_URL", "GEMINI_BASE_URL") ??
...
class AnthropicClient implements LlmClient {
  private defaultBaseUrl(): string {
    return normalizeBaseUrl(
      getEnvFirst("GITREVIEWPILOT_LLM_BASE_URL", "ANTHROPIC_BASE_URL") ?? "https://api.anthropic.com"
    );
  }
